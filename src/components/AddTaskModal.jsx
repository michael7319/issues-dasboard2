...
export default function AddTaskModal({ onClose, onAdd, taskToEdit }) {
  ...
  const [mainAssignee, setMainAssignee] = useState(taskToEdit?.mainAssignee || '');
  const [supportingAssignees, setSupportingAssignees] = useState(taskToEdit?.supportingAssignees || []);
  ...
  return (
    <div className="fixed inset-0 bg-white/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-gray-200 w-full max-w-lg p-6 rounded shadow-lg space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Main Assignee</label>
          <select
            className="w-full p-2 border rounded text-black"
            value={mainAssignee}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Supporting Assignees</label>
